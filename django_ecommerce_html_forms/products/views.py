...
def create_product(request):
    ...
    if request.method == 'GET':
        ...
        if errors:
            return render(request, 'create_product.html', context={'catagories':catagories, 'errors':errors, 'payload':request.POST})
        ...
        # <YOUR CODE HERE>
        price = request.POST.get('price')
        if float(price) > 10000 or float(price) < 0:
            errors['price'] = 'Price must be a positive float lower than 10000.00'
        ...
        product = Product.objects.create(name=name, sku=sku, price=price, category=category)  # <YOUR CODE HERE>
...
def edit_product(request, product_id):
    ...
    # static_form is just used as an example
        ...
        if errors:
            return render(request, 'edit_product.html', context={'catagories':catagories, 'errors':errors, 'payload':request.POST})
        ...
        product.name = request.POST.get('name')  # <YOUR CODE HERE>
        
        
        product.sku = request.POST.get('sku')
        
        product.price = request.POSTget('price')
